...
def plot(ys, ts=None, N=10):
    ...
    fig, axes = plt.subplots(
        ...
        sharex=True,
        sharey=True,
        figsize=(4 * plot_num_timesteps, 3),
    )
    ...
    for j in range(plot_num_timesteps):
        for i in range(N):
            for o in range(output_dim):
                axes[j].plot(x, ys[i, j, :, o], "-", ms=2, color="C0")
        if ts is not None:
...
fig, axes = plt.subplots(1, 2, figsize=(8 * 2, 8), tight_layout=True)
fig.subplots_adjust(wspace=0, hspace=0.0)

for k, rot in enumerate([I, R]):
    ys = jax.vmap(
        lambda key: conditional_sample(
            x_known @ rot.T, y_known @ rot.T, x_test @ rot.T, key=key
        )
    )(jax.random.split(key, num_samples))
    ys = ys @ rot
    ...
    covariances = jax.vmap(partial(jax.numpy.cov, rowvar=False), in_axes=[1])(ys)
    plot_cov(x_test, covariances, ax=axes[k])
plt.tight_layout()
plt.show()

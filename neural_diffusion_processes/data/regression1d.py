...
class GPFunctionalDistribution(FuntionalDistribution):
    ...
    def sample(self, key, x: Float[Array, "N 1"]) -> Float[Array, "N 1"]:
        return sample_prior_gp(
            ...
            params=self.params,
            x=x,
            obs_noise=self.params["noise_variance"],
        )
...
class Sawtooth(FuntionalDistribution):
    ...
    def sample(self, key, x: Float[Array, "N 1"]) -> Float[Array, "N 1"]:
        ...
        f = jax.random.uniform(fkey, (), minval=3.0, maxval=5.0)
        s = jax.random.uniform(skey, (), minval=-5.0, maxval=5.0)
        ks = jnp.arange(1, self.K_max + 1, dtype=x.dtype)[None, :]
        vals = (-1.0) ** ks * jnp.sin(2.0 * jnp.pi * ks * f * (x - s)) / ks
...
class Mixture(FuntionalDistribution):
    def __init__(self, generators: List[FuntionalDistribution]):
        assert (
            len(generators) == 4
        )  # see tests/test_mixture.py for a more general impl.
...
def get_batch(key, batch_size: int, name: str, task: str):
    ...
    x_context = task.x_context_dist.sample(
        seed=ckey, sample_shape=(batch_size, max_n_context, 1)
    )

    x_target = task.x_target_dist.sample(
        seed=tkey, sample_shape=(batch_size, max_n_target, 1)
    )
...
def data_generator(
    key, dataset, task, total_num_samples, batch_size, num_epochs: Optional[int] = None
):
    """
...
if __name__ == "__main__":
    ...
    def plot_data():
        ...
        for (i, dataset), (j, task) in itertools.product(
            enumerate(_DATASET_FACTORIES.keys()), enumerate(_TASK_CONFIGS.keys())
        ):
            print(dataset, task)
            ax = axes[i, j]
            ax.set_xlim(-4, 6)
            ...
            plot_data(
                data.xc, data.yc, data.xs, data.ys, ax, legend=(i == 0) and (j == 0)
            )
            if i == 0:
                ax.set_title(task)
            if j == 0:

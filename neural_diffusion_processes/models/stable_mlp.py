...
class NonLinearLayerWithResidualAndLayerNorm(hk.Module):
    def __init__(self, output_size: int, activation_fn: Callable = jax.nn.silu):
        ...
        self.layer_norm = hk.LayerNorm(
            axis=-1, create_offset=True, create_scale=True, param_axis=-1
        )
...
class StableMLP(hk.Module):
    ...
    def __init__(
        ...
        if not activate_final:
            assert (
                len(mlp_units) > 1
            ), "MLP is single linear layer with no non-linearity"
        ...
        if stable_layer:
            ...
            layers.extend(
                [
                    NonLinearLayerWithResidualAndLayerNorm(
                        layer_width, activation_fn=activation
                    )
            ...
            self.mlp_function = hk.nets.MLP(
                mlp_units, activate_final=True, activation=activation
            )
        ...
        if not activate_final:
            self.final_layer = (
                hk.Linear(n_output_params, b_init=jnp.zeros, w_init=jnp.zeros)
                if zero_init_output
                else hk.Linear(
                    n_output_params,
                    w_init=hk.initializers.VarianceScaling(
                        output_variance_scaling, "fan_avg", "uniform"
                    )
                    if output_variance_scaling
                    else None,
                )
            )

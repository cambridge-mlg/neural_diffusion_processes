...
cos_close_0 = {"function": jnp.cos, "coefficients": COS_TAYLOR_COEFFS}
sinc_close_0 = {
    "function": lambda x: jnp.sin(x) / x,
    "coefficients": SINC_TAYLOR_COEFFS,
}
inv_sinc_close_0 = {
    "function": lambda x: x / jnp.sin(x),
    "coefficients": INV_SINC_TAYLOR_COEFFS,
}
inv_tanc_close_0 = {
    "function": lambda x: x / jnp.tan(x),
    "coefficients": INV_TANC_TAYLOR_COEFFS,
}
cosc_close_0 = {
    "function": lambda x: (1 - jnp.cos(x)) / x**2,
    "coefficients": COSC_TAYLOR_COEFFS,
}
var_sinc_close_0 = {
    "function": lambda x: (x - jnp.sin(x)) / x**3,
    "coefficients": [-k for k in SINC_TAYLOR_COEFFS[1:]],
}
var_inv_tanc_close_0 = {
    "function": lambda x: (1 - (x / jnp.tan(x))) / x**2,
    "coefficients": VAR_INV_TAN_TAYLOR_COEFFS,
}
sinch_close_0 = {
    "function": lambda x: jnp.sinh(x) / x,
    "coefficients": SINHC_TAYLOR_COEFFS,
}
log_sinch_close_0 = {
    "function": lambda x: jnp.log(jnp.sinh(x) / x),
    "coefficients": LOG_SINHC_TAYLOR_COEFFS,
}
cosh_close_0 = {"function": jnp.cosh, "coefficients": COSH_TAYLOR_COEFFS}
inv_sinch_close_0 = {
    "function": lambda x: x / jnp.sinh(x),
    "coefficients": INV_SINHC_TAYLOR_COEFFS,
}
inv_tanh_close_0 = {
    "function": lambda x: x / jnp.tanh(x),
    "coefficients": INV_TANH_TAYLOR_COEFFS,
}
tanh_close_0 = {
    "function": lambda x: jnp.tanh(x) / x,
    "coefficients": TANH_TAYLOR_COEFFS,
}
log_tanh_close_0 = {
    "function": lambda x: jnp.log(jnp.tanh(x) / x),
    "coefficients": LOG_TANH_TAYLOR_COEFFS,
}
log1p_m_tanh_sq_close_0 = {
    "function": lambda x: jnp.log1p(-jnp.tanh(x) ** 2),
    "coefficients": LOG1P_M_TANH_SQ_TAYLOR_COEFFS,
}
arctanh_card_close_0 = {
    "function": lambda x: jnp.arctanh(x) / x,
...
def taylor_exp_even_func(point, taylor_function, order=5, tol=EPSILON):
    ...
    approx = jnp.einsum(
        "k,k...->...",
        jnp.array(taylor_function["coefficients"][:order]),
        jnp.array([point**k for k in range(order)]),
    )
    point_ = jnp.where(jnp.abs(point) <= tol, tol, point)
    exact = taylor_function["function"](jnp.sqrt(point_))

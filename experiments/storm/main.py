...
def run(config):
    ...
    def plots(state: TrainingState, key, t) -> Mapping[str, plt.Figure]:
        ...
        x_grid = jnp.linspace(0, plot_batch.xs.max(), 100)
        ...
        def plot_tracks(xs, ys, axes, title=""):
            for x, y in zip(xs, ys):
                ...
                m.plot(y[..., 1] / RADDEG, y[..., 0] / RADDEG, latlon=True, linewidth=0.3)


        fig_backward, axes = plt.subplots(1, 5, figsize=(8*5, 8*1), sharex=True, sharey=True)
        ...
        plot_tracks(x_grid, y_model, axes[2], rf"$p_{{model}}$")
        ...
        if t == 0: # NOTE: Only plot fwd at the beggining
            ...
            for k, t in enumerate(ts):
                ...
                plot_tracks(batch.xs, yt, axes[:, k+1], rf"$p_{{t={t}}}$")
